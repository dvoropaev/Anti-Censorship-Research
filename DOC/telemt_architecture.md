...
- `getProxySecret` (бинарный proxy-secret для ME-аутентификации)
- `getProxyConfig` (карта middle proxy endpoints, IPv4)
- `getProxyConfigV6` (карта middle proxy endpoints, IPv6)
...
- Парсер читает строки `proxy_for <dc> <ip:port>;`.
...
1. Встроенные таблицы констант (`TG_DATACENTERS_V4`, `TG_DATACENTERS_V6`, порт 443).
2. `dc_overrides` из конфига (если оператор явно переопределил).
